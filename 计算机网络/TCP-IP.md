...
---|---|---|--
1|物理层|定义物理设备标准|比特
2|数据链路层|定义了如何格式化数据并传输|帧
3|网络层|对端到端的包传输进行定义，还定义了如何将一个包分解为更小的包的分段方法|包
4|传输层|选择差错恢复协议还是无差错恢复协议；对收到的顺序不同的数据包进行重排序；对不同输入的数据流进行复用|段
...
TIME-WAIT的时长是2MSL，MSL的意思是“最大报文生存时长”，主动关闭方在第三次握手完毕，发送最后一个ack确认消息后，就会进入TIME-WAIT状态。等待2MSL是为了防止ACK响应丢失。（因为TCP是可靠的连接，每次的握手都要保证可靠性）。被动关闭方如果没有收到ack确认的话，就会重发第三次握手的FIN包。这时主动关闭方在接到这个FIN包后，可以再发一个ack应答包。另一方面是防止上次连接中的包影响到这次连接


>TINME-WAIT阶段两端的端口号都不能使用
>
>这里的主动关闭方不一定是客户端，比如爬虫服务器在爬取数据时自身就是主动关闭方。也有可能是server主动关闭连接，这样server就是主动关闭方

*	大量TIME-WAIT的问题

    占用系统资源和端口号，使得后面的连接无法进行。

*	解决方法

	缩短TIME-WAIT的时间；让系统能够快速回收和重用TIME-WAIT的资源(修改sysctl.conf的值)

### CLOSE-WAIT

就是被动关闭方收到关闭请求之后，没有返回ack确认，于是资源一直被占用着。解决方法只能是检查代码