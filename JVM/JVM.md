...
JMM，主要围绕多线程中出现的原子性，可见性，顺序性等问题及多线程通信的相关特性而建立的模型。Java**进程间通信**是通过共享内存实现的
...
存储局部变量表、操作数栈、常量池引用等信息
方法执行到完成的过程，对应着一个栈帧在java虚拟机栈中入栈和出栈的过程
...
<h3 id='2.1.2.1'>标记整理算法</h3>

*	与标记清除算法很像。整理阶段根据内存地址排序，末端内存地址之后的内存全部回收

<h3 id='2.1.2'>复制收集算法</h3>
...
<h3 id='2.1.3'>分代收集算法（现在的多说虚拟机采用方式）</h3>
...
*	软引用
	*	描述一些有用但非必须的对象，只有在内存不够的时候才会被回收

*	弱引用
	*	描述非必须对象，其存活时间只维持到下一次的GC之前，一定会被回收
...
>
>>标记-整理算法：标记阶段和标记-清除是一样的，整理阶段按照内存地址排序，最大地址之后的全部回收
...
*	FullGC
	...
	*	调用System.gc()
	*	老年代空间不足：尽量不要创建过大的对象或者数组 或者 通过虚拟机参数调整
	*	空间分配担保失败
	*	JDK1.7之前的永久代空间不足
...
<h2 id="7.3">基于栈的字节码解释执行引擎</h2>

java虚拟机执行引擎执行java代码时，有两种方式：
...
>java程序时半独立的实现，一部分动作在java虚拟机之外，而解释器在java虚拟机之内

<h3 id="7.3.2">基于栈的指令集与基于寄存器的指令集</h3>
...
*	基于寄存器的指令集（Intel开发，主流物理机）

<h3 id="7.3.3">基于栈的解释器执行过程</h3>

**概念模型和实际的虚拟机执行过程是有区别的，虚拟机在实际执行过程中会有优化**

****

<h1 id="8">八、类加载及执行子系统的案例与实战</h1>

只有 **字节码生成** 和 **类加载** 这两部分可以通过程序控制

<h2 id="8.1">案例分析</h2>

<h3 id="8.1.1">Tomcat：正统的类加载器架构</h3>

>Tomcat是Apache基金会中的一款开源Java Web服务器

![微信图片_20190603173157.jpg](.\微信图片_20190603173157.jpg)

<h3 id="8.1.2">灵活的类加载器架构</h3>

>EclipseIDE是基于 OSGI 规范实现的

不同于正统的类加载器架构，OSGI规范的类加载器是非树状继承的，这可能会产生循环依赖的问题，陷入死锁。解决办法可以是强制串行化。

**对于单个虚拟机下的应用，从开发初期就建立在OSGI上是一个不错的选择，便于约束依赖**

<h3 id="8.1.3">字节码生成技术与动态代理的实现</h3>

Spring中如果bean是面向接口编程，那么Spring内部是通过动态代理的方式来对bean进行增强的

<h3 id="8.1.4">Retroranslator：跨越JDK版本</h3>

用于在不同的JDK版本之间移植代码，将jdk1.5编译出来的Class文件转变为可以在JDK1.4或者JDK1.3上部署的版本

>JDK的每次升级大致分为4个方面：
>
*	在编译器层面的改进：如自动装箱，自动拆箱，变长参数，泛型等
*	对JavaAPI的增强
*	在字节码中进行支持的改进
*	虚拟机内部的改进

**上面这四个方面中，Retroranslator只能模仿前两类**

<h2 id="8.2">实战:自己动手实现远程执行功能</h2>
...
*	思路
	...
	*	如何收集java代码的执行结果
		...
		*	直接在执行的类中把对system.out的符号引用替换为printStream的符号引用

***

<h1 id="9">九、早期（编译期）优化</h1>

<h2 id="9.1">javac编译器</h2>
...
<h3 id="9.1.2">解析与填充符号表</h3>

*	词法分析
	>类似将`int a=b+c`分解为`int` `a` `=` `b` `+` `c`六个Token
*	语法分析
...
<h3 id="9.1.3">注解处理器</h3>

用于对注解进行处理

<h3 id="9.1.4">语义分析与字节码生成</h3>
...
*	语义分析
	...
	*	解语法糖
	>虚拟机在编译阶段，将不支持的语法还原为简单的基础语法结构的过程，称为解语法糖

*	字节码生成

	>不仅将前面阶段生成的各种信息（语法树，符号表）写入到磁盘中外，还完成了少量的代码添加与转换工作

<h2 id="9.2">java语法糖</h2>

>在计算机中添加语法，这种语法对于语言功能没有多大影响，但是会方便程序员的使用

**java中的语法糖主要有泛型，自动装、拆箱，变长参数等**

<h3 id="9.2.1">泛型与类型擦除</h3>

>java中的泛型只在程序源码中存在，在编译后的字节码文件中就转化为裸类型了（通过强制转型实现）

**泛型的实现称为类型擦除（就是简单的将泛型删除），丧失了泛型的优雅**

![泛型重载.png](.\泛型重载.png)

泛型重载，由于类型擦除的存在，导致两个方法相同，报错
加入不同的返回值即可共存于一个java class中

*	类型擦除

	>仅仅是对方法字节码中的code属性中的字节码进行擦除，实际上元数据还是保留了泛型信息，这也是通过反射取得参数化类型的基本依据

<h3 id="9.2.2">自动装箱，拆箱与遍历循环</h3>
...
>
>~~~java
>public static Integer show(Integer num){
	System.out.println("autoboxing example-method invocation i="+num);
    return num;
}
>
show(3);//自动装箱
...
**弊端在于，当循环存在时，自动装箱可能会创建出很多的原始类型（隐式的创建对象），引发性能问题**

**在使用“==”进行比较时，比较的应是基本或原始数据类型。当出现对象的比较时，应该使用equals()方法**

<h3 id="9.2.3">条件编译</h3>

>根据布尔值的真假，将分支中不成立的代码块消除掉

<h2 id="9.3">实战：插入式注解处理器</h2>


***

<h1 id="10">十、晚期（运行期）优化</h1>

>JIT编译器(即时编译器)，代码块->机器码（非必须）

[编译型语言与解释型语言的优缺点](https://www.jianshu.com/p/3b878e1019ef)

编译型语言在编译时将代码直接编译成机器码，执行速度较快，如c，c++
解释型语言在运行时动态将代码翻译成机器码，执行速度较慢，如java，c#
ps：虽然java也有编译的过程，但是是将代码编译为字节码而非机器码

内联：java即时编译器执行的优化

几项最具代表性的优化技术：

<h2 id="10.1">公共子表达式消除</h2>
<h2 id="10.2">数组边界检查消除</h2>

java对比C，C\++而言，好的一点在于，不是在裸指针上进行操作，因此可以运行许多检查，对于异常都有不同的抛出机制。不像c，c\++轻易出现Segment fault错误。但是，同样的操作，java的这些检查就会造成额外的系统开销，显得好像比较慢。需要尽可能的消除这些隐式开销，可以使用隐式异常处理，自动装箱消除等操作实现。


<h2 id="10.3">方法内联</h2>

>方法内联的优化行为看起来简单。不过是吧目标方法的代码“复制”到发起调用的方法之中，避免发起真实的方法调用。多数的java方法无法进行内联，因为其方法在运行期才进行方法选择

<h2 id="10.4">逃逸分析</h2>

是目前java虚拟机中比较前沿的优化技术

*	栈上分配

	*	一般情况下，是在java堆中为对象分配内存，堆中对象对于所有线程都是可见的，只要持有这个对象的引用，就可访问这个对象。GC可以回收堆中不在使用的内存，但是回收动作需要消耗系统资源。所以，如果确定一个对象不会逃逸出方法之外，可以将其在栈上分配空间。随着方法的出栈自动回收对象的内存。节省了系统开销

*	同步消除

	*	若确定变量不会逃逸出线程，则对其的同步措施可以消除
...
<h1 id="11">十一、Java内存模型与线程</h1>

<h2 id="11.4">Java与线程</h2>

>并发不一定依赖多线程：如PHP中的多进程并发

<h3 id="11.4.1">线程的实现</h3>

实现线程的三种方式：

*	使用内核线程实现
	>程序一般不直接使用内核线程，而是通过轻量级进程（一般意义上的线程）来实现，轻量级进程与线程是一一对应的关系

    问题在于：
    *	消耗较大（需要进行系统调用）
    *	内存资源（每个轻量级进程对应一个内核线程）

*	使用用户线程实现

	*	一对多。没有系统内核的支持，所有的线程操作需要用户自己处理。实现比较复杂，已经逐渐被抛弃。

*	使用用户线程加轻量级进程混合实现

	*	N*M关系，既存在用户线程，也存在轻量级进程

<h3 id="11.4.2">Java线程调度</h3>

*	协同式线程调度
	
    *	每个线程执行完成之后通知操作系统，自己执行完了
	*	优点：实现简单
	*	缺点：线程执行时间不可控制

*	抢占式线程调度

	*	由操作系统分配时间，当一个线程出现问题时，系统可能会将其 杀掉

<h3 id="11.4.3">状态转换</h3>

java的五种线程状态：

*	新建：创建后尚未启动
*	运行：可能正在执行，也可能正在等待CPU分配时间片
*	无限期等待（限期等待）：是否无限期区别在于是否需要等待其他线程显式唤醒
*	阻塞：区别于等待状态，阻塞状态在等待获取到一个排他锁
*	结束：线程终止

***

<h1 id="12">Java语言中的线程安全</h1>
...
2.	不直接阻塞自己，而是进入自旋状态
3.	自旋时重新获得锁
4.	若未获得锁，则在自旋结束后进入阻塞状态

优：锁持有时间长，但是竞争不激烈的情况
缺：需占用CPU，系统开销；单线程不适用

<h2 id="13.2">轻量级锁</h2>

**减少无实际竞争的情况下，使用重量级锁产生的系统消耗**

轻量级锁和重量级锁不是对立的

不需要申请互斥量，通过CAS操作实现，每次申请，释放锁都需要一次CAS

缺：如果锁竞争激烈，轻量级锁可能会膨胀为重量级锁

<h2 id="13.3">偏向锁</h2>

**减少无竞争情况下，维护轻量级锁产生的系统开销**

只有初始化时需要一次CAS

缺：若存在线程竞争，则膨胀为轻量级锁

内置所只能沿着 偏向锁->轻量级锁->重量级锁 的方式膨胀，不能逆向收缩。这是由于JVM的一个假定

>一旦破坏了上一级锁的假定，则此假定以后一直不会成立

## 比较两个类是否是同一个类

比较类的全限定名+是否是同一个类加载器加载

*	启动类加载器是JVM中的，本来就有
*	引导类加载器和应用程序类加载器继承自Java.lang.ClassLoader
*	当然也可以自定义
